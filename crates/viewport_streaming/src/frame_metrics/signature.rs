...
use super::FrameTransportMetrics;

#[derive(Debug, Default)]
pub(super) struct FrameSignatureMetrics {
    last_sequence: AtomicU64,
    last_hash: AtomicU64,
    last_mean_luma_bits: AtomicU64,
...
impl FrameSignatureMetrics {
    pub(super) fn record(&self, sequence: u64, hash: u64, mean_luma: f64, mad_luma: Option<f64>) {
        self.last_sequence.store(sequence, Ordering::Relaxed);
        self.last_hash.store(hash, Ordering::Relaxed);
        if mean_luma.is_finite() && mean_luma >= 0.0 {
            self.last_mean_luma_bits
                .store(mean_luma.to_bits(), Ordering::Relaxed);
        }
    ...
    pub(super) fn reset(&self) {
        ...
        self.last_mean_luma_bits.store(0, Ordering::Relaxed);
    ...
    pub(super) fn snapshot(
        &self,
    ) -> (Option<u64>, Option<u64>, Option<f64>, u64, u64, Option<f64>) {
        let frame_count = self.frame_count.load(Ordering::Acquire);
        let mad_frame_count = self.mad_frame_count.load(Ordering::Acquire);
        (
            (frame_count > 0).then(|| self.last_sequence.load(Ordering::Relaxed)),
            (frame_count > 0).then(|| self.last_hash.load(Ordering::Relaxed)),
            (frame_count > 0)
                .then(|| f64::from_bits(self.last_mean_luma_bits.load(Ordering::Relaxed))),
            frame_count,
            mad_frame_count,
            (mad_frame_count > 0)
...
impl FrameTransportMetrics {
    /// Returns the next render identity without reserving it.
    pub fn next_render_sequence(&self) -> u64 {
        self.inner
            .next_sequence
    ...
    pub fn record_frame_signature(
        &self,
        sequence: u64,
        hash: u64,
        mean_luma: f64,
        adjacent_mad_luma: Option<f64>,
    ) {
        self.inner
            .frame_signatures
            .record(sequence, hash, mean_luma, adjacent_mad_luma);
    }
}
