...
/// Inputs for refreshing an already-linked Scene while retaining its stable
/// Scene identity and every existing parent placement.
...
/// Transactionally refresh the source closure and wrapper for one linked
/// Scene. Existing canonical files remain in place until every replacement is
/// prepared; a failed publish restores the previous linked snapshot and
...
pub(crate) fn sync_linked_scene_atomic(
    ...
    let scene_name = request.name.trim();
        ...
        !scene_name.is_empty(),
        "synced Scene name must not be empty"
    );
    ensure!(
        request
            .base_manifest
            .scenes
            .iter()
            .any(|entry| entry.id == request.scene_id),
        "linked Scene target is not registered"
    );
    ...
    let result = (|| {
        ...
        let mut manifest_candidate = request.base_manifest.clone();
        if let Some(entry) = manifest_candidate
            .scenes
            .iter_mut()
            .find(|entry| entry.id == request.scene_id)
        {
            entry.display_name = scene_name.to_owned();
            entry.storage_key = StorageKey::new(scene_name.to_owned())?;
        }
        manifest_candidate
            .validate()
        ...
        if manifest_candidate != request.base_manifest.canonicalized() {
            ManifestStore::write_manifest_atomic(request.project_root, &manifest_candidate)
                .context("publish refreshed linked Scene manifest")?;
        }
        Ok(AdoptedScene {
