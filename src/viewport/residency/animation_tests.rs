use super::*;
use super::super::{ScenePayloadCatalog, ScenePayloadDescriptor};
...
fn catalog(
    ...
    let mut catalog = ScenePayloadCatalog {
    ...
    if let Some(address) = address {
        catalog.by_key.insert(
            key(scene_id, 1),
            vec![ScenePayloadDescriptor {
                address,
                prim_path: "/World/Node1".to_owned(),
                material: None,
                animation: Some(SceneCacheBlobRef {
                    blob_id: BlobId(key(scene_id, 1).blob_hash.to_hex()),
                    byte_size: 8,
                }),
            }],
        );
...
fn rebuild_requires_cacheable_owned_prim_animation_rows() {
    ...
    let cached_catalog = catalog(scene, 7, Some(presentation.entries[0].address.clone()));
    state.rebuild(&presentation, Some(&cached_catalog));
...
fn rebuild_requires_matching_scene_animation_payload_identity() {
    ...
    let cases = [
        ...
        (
            "wrong scene",
            Some(catalog(SceneId::new_v4(), 7, Some(entry.address.clone()))),
            0,
        ),
        (
            "wrong generation",
            Some(catalog(scene, 8, Some(entry.address.clone()))),
            0,
        ),
        ("missing address", Some(catalog(scene, 7, None)), 0),
        (
            "matching address",
            Some(catalog(scene, 7, Some(entry.address.clone()))),
            1,
        ),
    ];
...
#[test]
fn active_window_expansion_supplements_a_resident_non_animation_payload() {
    let scene = SceneId::new_v4();
    let key = key(scene, 1);
    let mut authority = ResidencyAuthority::default();
    let mut assets = Assets::<Mesh>::default();
    authority.install_scene(scene, 9, Vec::new());
    assert!(authority.request_reason(key, ResidencyReason::Selected, 9, 8, 8));
    let geometry = authority.begin_next_load().expect("geometry load");
    assert!(authority.complete_cpu(key, geometry.generation, 8, 8));
    assert_eq!(authority.pump_uploads(&mut assets, None), vec![key]);
    let resident_mesh = authority.render_asset_id(&key).expect("resident mesh");
    ...
    assert!(authority.begin_next_load().is_some());
